...
import type { StaticModalStore } from './types';

export const updateAndSaveModal =
  (modalStoreRef: MutableRefObject<StaticModalStore<any>[]>) => (instance: StaticModalStore<any>) => {
    const hasInstance = modalStoreRef.current.find(ins => ins.modalId === instance.modalId);
