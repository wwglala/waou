...
function Portal(props) {
    ...
    var getModal = function (Component) {
        if (React__default["default"].isValidElement(Component)) {
            ...
            return { Modal: config.Dialog, Component: React__default["default"].createElement(Component, null) };
        }
        var type = Component.type, Com = Component.Component;
        var Modal = type === "Drawer" ? config.Drawer : config.Dialog;
        var Container = typeof Com === "function" ? React__default["default"].createElement(Com, null) : Com;
        return { Modal: Modal, Component: Container };
    };
    return ReactDOM__default["default"].createPortal(React__default["default"].createElement(React__default["default"].Fragment, null, modalStore.map(function (item, key) {
        ...
        return (React__default["default"].createElement(Modal, __assign({ key: key, onClose: onClose, onCancel: onClose, onOk: onClose, visible: visible }, modalProps), React__default["default"].cloneElement(Com, __assign(__assign({}, props), { dispatch: dispatch, __onClose: onClose }))));
    })), modalRoot.current);
}
var ModalProvider = function (props) {
    var config = props.config, children = props.children;
    var modalContainer = React.useRef([]);
    var _a = React.useState(modalContainer.current), modalStore = _a[0], setModalStore = _a[1];
    var modalRoot = React.useRef(document.createElement("div"));
